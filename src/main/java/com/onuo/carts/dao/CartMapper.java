...
public interface CartMapper extends MyMapper<Cart> {
     ...
     */
      int save(ShoesDetail shoesDetail, String custId);
    /**
     @param shoesDetail 要从购物车中删除的鞋对象
     @param custId 当前用户ID
     @return 返回是否删除成功,true表示删除成功，false表示删除失败
     */
    int delete(ShoesDetail shoesDetail,String custId);
    /**
     ...
     @return 返回是否删除成功,true表示删除成功，false表示删除失败
     */
    int updateCount(String shoes_detail_id,String custId,Integer count);

...
public interface ReceiveAddressMapper extends MyMapper<ReceiveAddress> {
     ...
     */
      boolean save(ReceiveAddress newAddress,Integer custId);
     ...
     */
      boolean delete(Integer addressId,Integer custId);
     ...
     */
      boolean update(ReceiveAddress address,Integer custId);
    /**
     @param custId 当前用户ID
     @return 返回当前客户所有的
     */
    ...
    /**
     ...
     @return 返回是否修改成功,true表示修改成功，false表示修改失败
     */
      boolean updateDefaultAddress(Integer addressId,Integer custId);
}